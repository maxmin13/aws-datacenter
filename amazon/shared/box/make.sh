...
then
   ...
   sgp_id="$(create_security_group "${dtc_id}" "${SHARED_BOX_SEC_GRP_NM}" 'Shared security group.')"  
...
granted_ssh_38142="$(check_access_from_cidr_is_granted  "${sgp_id}" "${SHARED_BOX_SSH_PORT}" '0.0.0.0/0')"

if [[ -z "${granted_ssh_38142}" ]]
then
   allow_access_from_cidr "${sgp_id}" "${SHARED_BOX_SSH_PORT}" '0.0.0.0/0'
   
   echo "Granted SSH access on port ${SHARED_BOX_SSH_PORT}."
fi
...
hashed_pwd="$(mkpasswd --method=SHA-512 --rounds=4096 "${SHARED_BOX_USER_PWD}")" 
...
awk -v key="${public_key}" -v pwd="${hashed_pwd}" -v user="${SHARED_BOX_USER_NM}" -v hostname="${SHARED_BOX_HOSTNAME}" '{
...
ssh_port="$(get_ssh_port "${key_pair_file}" "${eip}" "${SHARED_BOX_USER_NM}" '22' '38142' )"
...
remote_dir=/home/"${SHARED_BOX_USER_NM}"/script
...
scp_upload_files "${key_pair_file}" "${eip}" "${ssh_port}" "${SHARED_BOX_USER_NM}" "${remote_dir}" \
...
wait_ssh_started "${key_pair_file}" "${eip}" "${SHARED_BOX_SSH_PORT}" "${SHARED_BOX_USER_NM}"

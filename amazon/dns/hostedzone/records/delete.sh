...
#######################################################
# The script deletes the DNS record that points to the 
# Admin website and a DNS record alias that points to 
# the load balancer.
#######################################################
...
check_hosted_zone_exists "${MAXMIN_TLD}" > /dev/null
hosted_zone_exists="${__RESULT}"

if [[ 'false' == "${hosted_zone_exists}" ]]
then
